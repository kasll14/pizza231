...
class OrderController
{
    ...
    public function process(): void
    {
        ...
        // 📧 ОТПРАВКА EMAIL - ГЛАВНАЯ ЧАСТЬ
        $emailSent = $this->sendOrderEmail($order);
        $this->sendAdminNotification($order);
        
        // Логируем результат отправки (для отладки)
    ...
    private function sendOrderEmail(array $order): bool
    {
        ...
        // 3. Формируем тему письма
        $subject = 'Заказ №' . $order['id'] . ' подтверждён - ' . self::FROM_NAME;
        
        // 4. Генерируем HTML-тело письма
        $message = $this->buildCustomerEmailBody($order);
        
        // 5. Настраиваем заголовки для HTML-письма
        $headers = $this->getEmailHeaders();
        
        // 6. Отправляем письмо функцией mail()
        $result = @mail($to, $subject, $message, $headers);
        
        // 7. Возвращаем результат (true/false)
        return $result;
    }
                    ...
                    <tr>
                        <td style="padding: 30px;">
                            <table role="presentation" style="width: 100%; background: #ebf4ff; border-radius: 8px; padding: 20px;">
                                <tr>
                                    <td>
                                        <p style="margin: 0 0 10px 0; color: #2c5282; font-size: 14px; font-weight: 600;">
                                            📋 ИНФОРМАЦИЯ О ЗАКАЗЕ
                                        </p>
